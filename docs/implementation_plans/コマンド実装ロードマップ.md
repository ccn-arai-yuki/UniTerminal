...
| コマンド | 説明 | 計画書 |
|---------|------|--------|
| `head` | ファイルの先頭を出力 | [head_command_implementation_plan.md](head_command_implementation_plan.md) |
| `tail` | ファイルの末尾を出力（-fでリアルタイム監視） | [tail_command_implementation_plan.md](tail_command_implementation_plan.md) |
| `log` | Unityログの表示・監視 | [log_command_implementation_plan.md](log_command_implementation_plan.md) |

**前提機能:**
| 機能 | 説明 | 計画書 |
|------|------|--------|
| Ctrl+Cキャンセル | コマンド実行中のキャンセル機能 | [ctrl_c_cancel_implementation_plan.md](ctrl_c_cancel_implementation_plan.md) |
...
### Phase 6: ファイル・ログ監視コマンド

**目標:** ファイルの先頭・末尾表示、Unityログ監視機能を実装

#### 6.1 基盤機能

```
0. Ctrl+Cキャンセル機能 [★★☆] - コマンド実行中のキャンセル（-fオプションの前提）
```

**変更ファイル:**
- `InputHandler.cs` - `IsPressedCtrlC()`追加
- `UniTerminal.cs` - キャンセル機構追加

#### 6.2 コマンド実装

```
1. head      [★☆☆] - ファイルの先頭を出力（-n, -c, -q, -v オプション）
2. tail      [★★☆] - ファイルの末尾を出力（-f でリアルタイム監視）
3. log       [★★☆] - Unityログの表示・監視（LogBuffer + CircularBuffer）
```

**Phase 6 完了条件:**
- [x] Ctrl+Cキャンセル機能実装・テスト完了 ✅
- [x] headコマンド実装・テスト完了 ✅
- [x] tailコマンド実装・テスト完了 ✅
- [x] LogBuffer実装・テスト完了 ✅
- [x] logコマンド実装・テスト完了 ✅
...
```
   ...
   ┌─────────────────────────────────────────────────────┐
   │              Ctrl+Cキャンセル機能                    │
   │  (InputHandler.IsPressedCtrlC, UniTerminal修正)     │
   └────────────────────────┬────────────────────────────┘
                            │
            ┌───────────────┼───────────────┐
            │               │               │
...
| Phase 5 | - | ★★☆ | 各コマンドの拡張 |
| Phase 6 | 3 | ★★☆ | Ctrl+Cキャンセル機能、LogBuffer含む |
...
| 2025-01-27 | Phase 6 追加（head, tail, log コマンド、Ctrl+Cキャンセル機能） |
| 2025-01-27 | Phase 6 完了（Ctrl+Cキャンセル、head, tail, LogBuffer, log コマンド実装） |
